...
describe('ISO-REL001', () => {
  ...
  it('flags empty catch block', async () => {
    writeAndCommit(tmpDir, 'src/a.ts', `
try {
  doThing()
} catch (e) {
}
`)
  ...
  it('flags comment-only catch block', async () => {
    writeAndCommit(tmpDir, 'src/a.ts', `
try { doThing() } catch (e) {
  // expected
}
...
`)
  ...
  it('flags empty Java catch block', async () => {
    writeAndCommit(tmpDir, 'Foo.java', `
public class Foo {
  void run() {
    try {
      doThing();
...
`)
  ...
  it('flags except: pass', async () => {
    writeAndCommit(tmpDir, 'utils.py', `
try:
    do_thing()
except:
    pass
...
`)
  ...
  it('ignores non-source files', async () => {
    writeAndCommit(tmpDir, 'README.md', `
\`\`\`
try { x() } catch (e) {}
\`\`\`
`)
