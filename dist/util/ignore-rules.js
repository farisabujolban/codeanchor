...
function isCommentedOutCode(text) {
    ...
    if (lines.length === 0)
        return false;
    const codeLines = lines.filter(line => CODE_HEURISTIC_PATTERNS.some(p => p.test(line)));
    return codeLines.length / lines.length > 0.5;
}
export function shouldIgnoreComment(comment, driver) {
    const text = comment.text;
    if (driver.directivePatterns.some(p => p.test(text)))
        return true;
    if (LICENSE_PATTERN.test(stripCommentMarkers(text)))
        return true;
    if (TODO_ONLY_PATTERN.test(stripCommentMarkers(text)))
        return true;
    if (isCommentedOutCode(text))
        return true;
    return false;
}
