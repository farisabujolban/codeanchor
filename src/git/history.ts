...
export function getHotFiles(repoRoot: string, since: string, minCommits: number = 3): HotFile[] {
...
export function getFileCommitCount(repoRoot: string, filePath: string, since: string): number {
    const dateStr = parseSinceDuration(since);
    try {
        const out = execFileSync('git', ['log', `--since=${dateStr}`, '--oneline', '--', filePath], {
            encoding: 'utf-8',
            cwd: repoRoot,
        });
