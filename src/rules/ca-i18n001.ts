...
function flattenLocaleKeys(obj: unknown, prefix: string, keys: Set<string>): void {
    const stack: Array<[unknown, string]> = [[obj, prefix]];
    while (stack.length > 0) {
        const [cur, pfx] = stack.pop()!;
        if (typeof cur !== 'object' || cur === null || Array.isArray(cur)) continue;
        for (const [k, v] of Object.entries(cur as Record<string, unknown>)) {
            const full = pfx ? `${pfx}.${k}` : k;
            keys.add(full);
            stack.push([v, full]);
        }
    }
...
export const caI18n001: Rule = {
    id: 'CA-I18N001',
    description:
        'A static i18n key used in code is not present in any locale file. (JS/TS/Vue/Svelte — add entries to DETECTORS array to support other languages.)',
