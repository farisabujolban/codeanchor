...
function findEmptyCStyleCatches(content: string): EmptyCatch[] {
    ...
    while (i < lines.length) {
        ...
        // Skip comment lines — avoids false positives when 'catch {}' appears in a comment
        ...
        if (closedOnSameLine) {
            ...
            if (stripped.length === 0 && !hasComment) results.push({ line: i + 1 });
            i = j + 1;
            continue;
        }
...
export const isoRel001: Rule = {
    ...
    async run(ctx: RuleContext): Promise<Finding[]> {
        ...
        for (const relPath of filePaths) {
            ...
            const analysisContent = JS_TS_EXTS.has(ext) ? stripTemplateLiterals(content) : content;
            const matches = isCStyle
                ? findEmptyCStyleCatches(analysisContent)
                : findEmptyPythonExcepts(analysisContent);
