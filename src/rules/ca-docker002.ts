...
function extractScriptNamesFromRun(runLine: string): string[] {
  const names: string[] = []
...
function extractNodePathFromExec(cmd: string): string | null {
...
function parseCmdEntrypoint(trimmed: string): string {
  // JSON array form: CMD ["node", "dist/index.js"]
  const jsonMatch = trimmed.match(/^(?:CMD|ENTRYPOINT)\s+(\[.*\])\s*$/i)
  if (jsonMatch) {
    try {
      const arr = JSON.parse(jsonMatch[1]) as string[]
      return arr.join(' ')
