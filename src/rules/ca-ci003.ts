...
// Uses character-class quantifiers (not group quantifiers) to avoid nested-quantifier ReDoS.
const LOCAL_PATH = /((?:\.{1,2}\/)?[^\s'"`\n]*\/[^\s'"`\n]*\.\w+)/;
...
function extractRunPaths(runBlock: string): string[] {
...
export const caCi003: Rule = {
    ...
    async run(ctx: RuleContext): Promise<Finding[]> {
        ...
        for (const wfFile of findWorkflowFiles(ctx.repoRoot)) {
            ...
            function report(localPath: string, hint: string): void {
            ...
            for (const job of Object.values(doc.jobs)) {
                for (const step of job.steps ?? []) {
                    ...
                    if (
                        typeof wd === 'string' &&
                        (wd.startsWith('./') || wd.startsWith('../') || (!wd.startsWith('/') && !wd.includes('$')))
                    ) {
                    ...
                    if (step.with && typeof step.with === 'object') {
                        for (const key of ['path', 'cache-dependency-path'] as const) {
