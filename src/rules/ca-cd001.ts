...
export const caCd001: Rule = {
    ...
    async run(ctx: RuleContext): Promise<Finding[]> {
        ...
        if (ruleCfg === false) return [];
        const maxOwnershipDistance =
            typeof ruleCfg === 'object' && ruleCfg?.maxOwnershipDistance ? ruleCfg.maxOwnershipDistance : 20;
        ...
        for (const fileDiff of stagedDiffs) {
            ...
            if (fileDiff.status === 'added') continue;

            const driver = getDriver(fileDiff.path);
            if (!driver) continue;

            if (isExcluded(fileDiff.path, config.exclude)) continue;
            ...
            for (const comment of comments) {
                if (shouldIgnoreComment(comment, driver)) continue;
                ...
                if (commentChanged) continue;

                const approval = findApproval(approvalsStore, fileDiff.path, comment.startLine);
                if (approval && isApprovalValid(approval, comment, lines, region)) continue;

                const changedInRegion: number[] = [];
                for (let l = region.startLine; l <= region.endLine; l++) {
                    if (fileDiff.changedLines.has(l)) changedInRegion.push(l);
                }
                const rangeStr =
                    changedInRegion.length === 1
                        ? `${changedInRegion[0]}`
