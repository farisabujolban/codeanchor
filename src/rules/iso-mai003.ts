  ...
  async run(ctx: RuleContext): Promise<Finding[]> {
    ...
    for (const relPath of trackedFiles) {
      ...
      for (const specifier of extractSpecifiers(content)) {
        ...
        if (!importers) { importers = new Set(); afferentMap.set(resolved, importers) }
    ...
    for (const [importedFile, importers] of afferentMap) {
      ...
      if (importers.size > threshold) {
        findings.push({
          ...
          fix: 'Consider splitting this module into smaller, more focused units to reduce the number of dependents, ' +
