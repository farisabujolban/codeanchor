...
const PARAM_DETECTORS: ParamDetector[] = [
    {
        ...
        routeStartPattern:
            /\.(?:get|post|put|patch|delete|options)\s*\(\s*['"]([^'"]+)['"]\s*,\s*(?:async\s*)?\([^)]*\)\s*=>\s*\{/gi,
...
export const caRoute001: Rule = {
    id: 'CA-ROUTE001',
    description:
        'URL path parameter name does not match the req.params key accessed in the inline handler — silent undefined at runtime. (Express/Fastify in v1 — add entries to PARAM_DETECTORS for Flask, Gin, Django etc.)',
