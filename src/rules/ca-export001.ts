...
function parseJsTsExports(content: string): Set<string> {
  ...
  for (const m of content.matchAll(/^export\s+(?:default\s+)?(?:(?:abstract\s+)?class|function\*?|const|let|var|type|interface|enum)\s+(\w+)/gm)) {
    symbols.add(m[1])
  }
...
export const caExport001: Rule = {
  id: 'CA-EXPORT001',
  description: 'A symbol was silently removed from a public module index file — potential breaking change if this package is published. (JS/TS in v1; extend via INDEX_DETECTORS.)',
  ...
  async run(ctx: RuleContext): Promise<Finding[]> {
    ...
    for (const detector of INDEX_DETECTORS) {
      for (const relIndexPath of detector.fileNames) {
        ...
        try { currentContent = fs.readFileSync(absPath, 'utf-8') } catch { continue }
