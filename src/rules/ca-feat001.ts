...
export const caFeat001: Rule = {
    id: 'CA-FEAT001',
    description:
        'A feature flag key referenced in code is not declared in the flags config file. Requires configFile in rule config. Works for any language — configure pattern to match your SDK.',
    ...
    async run(ctx: RuleContext): Promise<Finding[]> {
        ...
        try {
            pattern =
                typeof cfg.pattern === 'string' ? new RegExp(cfg.pattern, 'g') : new RegExp(DEFAULT_PATTERN_SRC, 'g');
        } catch {
            pattern = new RegExp(DEFAULT_PATTERN_SRC, 'g');
        }
