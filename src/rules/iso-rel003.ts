...
// Float literal with a non-zero fractional part, e.g. 0.5, 3.14, 1.5 but NOT 1.0 or 0.0
// Pattern: digits . digits where fractional part contains at least one non-zero digit
const FLOAT_RE = /(?:\d+\.(?:\d*[1-9]\d*)|(?:\d*[1-9]\d*)\.\d+)/

// JS/TS/Java/C equality operators
const CSTYLE_EQ_OP = /===|!==|==|!=/

// Java float/double suffix
const JAVA_SUFFIX = /[fFdD]?/

// Python uses == and != only (no === or !==)
const PYTHON_EQ_OP = /==|!=/
  ...
  async run(ctx: RuleContext): Promise<Finding[]> {
    ...
    for (const relPath of filePaths) {
      ...
      for (let i = 0; i < lines.length; i++) {
        const stripped = isCStyle
          ? stripLineComment(lines[i])
          : stripHashComment(lines[i])

        const match = isCStyle
          ? checkCStyleLine(stripped)
          : checkPythonLine(stripped)
