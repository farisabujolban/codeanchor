...
export const caDocker005: Rule = {
    ...
    async run(ctx: RuleContext): Promise<Finding[]> {
        ...
        const checks: Array<{ target: string; severity: 'error' | 'warn'; message: string; fix: string }> = [
            {
                ...
                severity: 'warn',
                message:
                    '.dockerignore does not exclude "node_modules" — gigabytes will be sent to the Docker daemon on every build.',
            ...
            },
            {
                ...
                severity: 'warn',
                message:
                    '.dockerignore does not exclude ".git" — full repository history is sent to the daemon and may end up in the image.',
        ...
        for (const check of checks) {
            ...
            const covered =
                check.target === '.env'
                    ? patternCovers(lines, '.env') ||
                      patternCovers(lines, '.env*') ||
                      lines.some((l) => /^\.env/.test(l))
                    : check.target === '*.pem'
