...
function findPythonBroadExcepts(content: string): BroadCatch[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while (j < lines.length) {
      ...
      if (trimmed.length === 0) { bodyLines.push(l); j++; continue }  // blank
      if (trimmed.startsWith('#')) { bodyLines.push(l); j++; continue }  // comment
    ...
    if (/\braise\b/.test(bodyText)) { i = j - 1; continue }
