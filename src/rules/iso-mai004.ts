...
const BARREL_BASENAMES = new Set([
  ...
  'mod.ts', 'mod.js', '__init__.py',
])

// Path segments that indicate a file IS a test (don't flag it as needing a test)
const TEST_PATH_MARKERS = ['__tests__', '.test.', '.spec.', '_test.', 'Test.java', 'Spec.java']
...
function testCandidates(relPath: string): string[] {
  ...
  if (ext === '.java') {
    return [
      `${prefix}${stem}Test.java`,
      `${prefix}${stem}Spec.java`,
    ]
  }
