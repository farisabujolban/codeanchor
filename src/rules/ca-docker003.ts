...
interface RuntimeRef { major: number; source: string }
...
export const caDocker003: Rule = {
  id: 'CA-DOCKER003',
  description: 'Dockerfile FROM runtime version does not match .nvmrc, .python-version, go.mod, or package.json engines.',
  ...
  async run(ctx: RuleContext): Promise<Finding[]> {
    ...
    for (const dockerFile of findDockerfiles(ctx.repoRoot)) {
      ...
      let content: string
      try { content = fs.readFileSync(dockerFile, 'utf-8') } catch { continue }
