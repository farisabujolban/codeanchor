...
/**
 * DFS-based cycle detection using an explicit call stack (iterative).
 * Each frame tracks the node and which neighbor index to visit next.
 */
function detectCycles(graph: Map<string, string[]>): string[][] {
    ...
    for (const startNode of graph.keys()) {
        ...
        const callStack: Array<{ node: string; neighborIdx: number }> = [{ node: startNode, neighborIdx: 0 }];
        ...
        while (callStack.length > 0) {
            ...
            if (frame.neighborIdx === 0) {
                // First visit: apply entry logic
                if (pathStack.length > MAX_PATH_DEPTH) {
                    callStack.pop();
                    continue;
                }
                if (onStack.has(node)) {
            ...
            if (frame.neighborIdx < neighbors.length) {
                ...
                // All neighbors visited: apply exit logic
                pathStack.pop();
                onStack.delete(node);
                callStack.pop();
            }
...
export const isoMai001: Rule = {
    ...
    async run(ctx: RuleContext): Promise<Finding[]> {
        ...
        for (const relPath of trackedFiles) {
            ...
            const specifiers = isJsTs ? extractJsTsSpecifiers(content) : extractPythonSpecifiers(relPath, content);

            const edges: string[] = [];
            for (const spec of specifiers) {
                const resolved = isJsTs
                    ? resolveSpecifier(relPath, spec, trackedSet)
                    : trackedSet.has(spec + '.py')
                      ? spec + '.py'
                      : trackedSet.has(spec)
                        ? spec
                        : null;
