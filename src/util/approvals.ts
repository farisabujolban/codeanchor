...
export function findApproval(store: ApprovalsStore, filePath: string, commentStartLine: number): Approval | null {
    return store.approvals.find((a) => a.file === filePath && a.commentLine === commentStartLine) ?? null;
}

export function isApprovalValid(approval: Approval, comment: Comment, lines: string[], region: OwnedRegion): boolean {
