...
program.name('codeanchor').description('Deterministic tech-debt and workflow-drift CLI').version('0.1.0');
    ...
    .action(async (opts) => {
        ...
        if (opts.json !== undefined) {
            ...
            if (typeof opts.json === 'string') {
                await fs.promises.writeFile(opts.json, json, 'utf-8');
            } else {
        ...
        if (opts.markdown !== undefined) {
            ...
            if (typeof opts.markdown === 'string') {
                await fs.promises.writeFile(opts.markdown, md, 'utf-8');
            } else {
